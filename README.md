...
이를 통해 동일한 환경을 다른 시스템에서 재현할 수 있습니다.
...
- **빌드 관점**: 이미지는 Dockerfile로부터 빌드되어 불변의 템플릿 역할을 합니다. 컨테이너는 이미지의 인스턴스로, 실행 시 생성됩니다.
- **실행 관점**: 이미지는 실행되지 않으며, 컨테이너가 실제로 실행되어 프로세스를 호스팅합니다.
- **변경 관점**: 이미지는 읽기 전용으로 변경할 수 없으며, 컨테이너는 실행 중 파일 시스템 변경이 가능하지만 컨테이너 삭제 시 사라집니다.
...
컨테이너 내부 포트(예: 80)로 직접 접속할 수 없는 이유는 컨테이너가 격리된 네트워크 환경에서 실행되기 때문입니다. 호스트와 컨테이너 간 네트워크가 분리되어 있어, 호스트에서 컨테이너의 내부 포트에 직접 접근할 수 없습니다.

포트 매핑(-p 옵션)이 필요한 이유는 호스트의 포트(예: 8081)를 컨테이너의 포트(80)에 연결하여 외부에서 컨테이너의 서비스에 접근할 수 있도록 하기 위함입니다. 이를 통해 웹 서버 등의 애플리케이션을 호스트 브라우저에서 테스트할 수 있습니다.
...
이러한 방법을 통해 컨테이너 라이프사이클과 데이터 라이프사이클을 분리할 수 있습니다.
...
스크립트의 이식성과 명확성을 고려하여 상황에 맞게 선택합니다.
...
파일 권한은 8진수로 표기되며, 각 숫자는 rwx(읽기/쓰기/실행) 권한의 조합을 나타냅니다:
- 4: 읽기 (r)
- 2: 쓰기 (w)  
- 1: 실행 (x)
- 0: 권한 없음
...
권한은 보안과 기능 요구사항에 따라 결정되며, 실행 파일은 755, 설정 파일은 644 등으로 설정합니다.
